...
import { PERSONAL_DEMO_PROMISES } from "@/lib/data/personal-demo";
...
export default function PersonalDemoPage() {
  const [promises, setPromises] = useState<PersonalPromise[]>(PERSONAL_DEMO_PROMISES);
  ...
  const handleReflect = useCallback(
    (id: string, reflection: string) => {
      setPromises((prev) => prev.map((p) => (p.id === id ? { ...p, reflection } : p)));
    },
    []
  );
      ...
      <main id="main-content" className="mx-auto max-w-3xl px-4 py-6">
        <div className="mb-6">
          ...
          <p className="mt-2 rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-700">
            This is a demo with sample data. Status buttons are interactive — try marking a promise
            as verified or degraded.
          </p>
        </div>
