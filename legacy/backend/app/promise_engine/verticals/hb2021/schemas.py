...
EMISSIONS_TARGET = PromiseSchema(
    ...
    schema_json={
        ...
        "properties": {
            ...
            "reporting_year": {"type": "integer", "minimum": 2022, "description": "Calendar year of emissions data"},
            ...
            "actual_reduction_pct": {"type": "number", "description": "Actual % reduction achieved from baseline"},
            "data_source": {"type": "string", "description": "DEQ filing reference or docket number"},
        },
        "required": [
            "utility_id",
            "reporting_year",
...
CLEAN_ENERGY_PLAN = PromiseSchema(
    ...
    schema_json={
        "type": "object",
        "properties": {
            "utility_id": {"type": "string", "enum": ["pge", "pacificorp"]},
            "irp_cycle_year": {"type": "integer", "description": "IRP cycle year"},
            "cep_filing_date": {"type": "string", "format": "date", "description": "Date CEP was filed with PUC"},
            "cep_docket_number": {"type": "string", "description": "PUC docket number"},
            "puc_disposition": {
                "type": "string",
    ...
    verification_rules={
        "rules": [
            ...
            {"if": {"puc_disposition": "pending"}, "result": "pending", "reason": "CEP under PUC review"},
        ]
    },
    training_eligible=True,
...
COMMUNITY_BENEFITS = PromiseSchema(
    ...
    schema_json={
        ...
        "properties": {
            "utility_id": {"type": "string", "enum": ["pge", "pacificorp"]},
            "assessment_period_start": {"type": "string", "format": "date"},
            "assessment_period_end": {"type": "string", "format": "date"},
            "advisory_group_convened": {"type": "boolean", "description": "Whether UCBIAG has been convened"},
            ...
            "cbre_mw_capacity": {"type": "number", "minimum": 0, "description": "Total MW capacity of CBRE projects"},
            "local_jobs_created": {"type": "integer", "minimum": 0},
            "rate_impact_on_low_income_pct": {
                "type": "number",
                "description": "Rate change impact on low-income ratepayers (%)",
            },
            "resiliency_projects_count": {"type": "integer", "minimum": 0},
            "advisory_group_recommendations": {"type": "array", "items": {"type": "string"}},
            "recommendations_adopted_count": {"type": "integer", "minimum": 0},
        },
...
LABOR_STANDARDS = PromiseSchema(
    ...
    schema_json={
        ...
        "properties": {
            ...
            "utility_id": {"type": "string", "enum": ["pge", "pacificorp", "ess"]},
            "capacity_mw": {"type": "number", "minimum": 0, "description": "Project capacity in megawatts"},
            "requires_labor_standards": {"type": "boolean", "description": "True if capacity > 10 MW"},
            "project_labor_agreement": {"type": "boolean", "description": "Whether a PLA is in place"},
            "prevailing_wage_paid": {"type": "boolean", "description": "Whether prevailing wage is being paid"},
...
RATE_IMPACT = PromiseSchema(
    ...
    schema_json={
        ...
        "properties": {
            "utility_id": {"type": "string", "enum": ["pge", "pacificorp"]},
            "rate_year": {"type": "integer"},
            "annual_revenue_requirement_usd": {"type": "number", "minimum": 0},
            "compliance_cost_usd": {
                "type": "number",
                "minimum": 0,
                "description": "Incremental cost of HB 2021 compliance",
            },
            "rate_impact_pct": {"type": "number", "description": "Compliance cost as % of revenue requirement"},
            "exceeds_cap": {"type": "boolean", "description": "Whether rate impact exceeds 6%"},
            "exemption_requested": {"type": "boolean"},
            "exemption_granted": {"type": "boolean"},
            "exemption_reason": {
                "type": "string",
    ...
    verification_rules={
        "rules": [
            {"condition": "rate_impact_pct <= 6.0", "result": "kept", "reason": "Rate impact within 6% statutory cap"},
            {
...
FOSSIL_FUEL_BAN = PromiseSchema(
    ...
    schema_json={
        ...
        "properties": {
            ...
            "new_gas_plants_proposed": {"type": "integer", "minimum": 0},
            "new_gas_plants_permitted": {"type": "integer", "minimum": 0},
            "existing_gas_expansions_proposed": {"type": "integer", "minimum": 0},
            "existing_gas_expansions_permitted": {"type": "integer", "minimum": 0},
        },
